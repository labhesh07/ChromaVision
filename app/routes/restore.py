...
from app.services.ml_shared import run_restore
...
@router.post("/process")
async def process_restore(file: UploadFile = File(...)) -> StreamingResponse:
...
async def process_restore_json(file: UploadFile = File(...)) -> JSONResponse:
    settings = get_settings()
    try:
